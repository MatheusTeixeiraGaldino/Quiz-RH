...
export default function PlayerGame() {
  ...
  useEffect(() => {
    ...
    const u = onSnapshot(q, (snap) => {
      ...
      setQuestions(qs)
      if (qs.length > 0) setDataReady(true)
    })
    return u
  }, [roomId])

  // ── Subscribe to my answers ────────────────────────────────────────────────
  useEffect(() => {
  ...
  useEffect(() => {
    if (!room || room.status !== 'playing' || questions.length === 0) {
    ...
    if (prevQIdx.current !== cqIdx) {
      prevQIdx.current = cqIdx
      setPhase('question')
  ...
  const answer = async (idx) => {
    ...
    const timeBonus = isCorrect ? Math.max(0, Math.round((timer / maxTime) * q.pontuacao * 0.5)) : 0
    ...
    if (soundEnabled) {
      try {
        ...
        if (isCorrect) {
          ...
          osc.frequency.setValueAtTime(180, ctx.currentTime + .15)
          gain.gain.setValueAtTime(.3, ctx.currentTime)
    ...
    } catch (e) { console.error('answer write error', e) }
  }

  // ── Loading ────────────────────────────────────────────────────────────────
  if (!room || !dataReady) {
    return (
      <div className="game-bg" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ textAlign: 'center', color: '#fff' }}>
          <div className="spinner" style={{ borderColor: 'rgba(255,255,255,.3)', borderTopColor: '#fff', marginBottom: 12 }} />
          <p style={{ fontWeight: 700, opacity: .8 }}>Carregando…</p>
        </div>
      </div>
    )
  }
  ...
  if (room.status === 'finished') {
    return (
      <div className="game-bg" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 24, gap: 20, textAlign: 'center' }}>
        ...
        <div className="score-box" style={{ width: 240 }}>
          <div style={{ fontSize: 12, fontWeight: 800, textTransform: 'uppercase', letterSpacing: 1, color: 'rgba(255,255,255,.7)', marginBottom: 4 }}>Pontuação final</div>
          <div className="score-num">{myScore.toLocaleString('pt-BR')}</div>
        </div>
        <button onClick={() => navigate(`/room/${roomId}/podium`)} className="btn btn-white" style={{ maxWidth: 280, fontSize: 17 }}>🏆 Ver pódio</button>
        <button onClick={() => navigate('/')} className="btn btn-secondary" style={{ maxWidth: 280, background: 'rgba(255,255,255,.15)', color: '#fff', borderColor: 'rgba(255,255,255,.3)' }}>🏠 Início</button>
      </div>
    )
  }
  ...
  if (!q) {
    return (
      <div className="game-bg" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 16, color: '#fff', textAlign: 'center' }}>
  ...
  if (phase === 'feedback' && hasAns) {
    return (
      <div className={isOk ? 'feedback-ok' : 'feedback-ko'} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 24, textAlign: 'center', gap: 12 }}>
        <div style={{ fontSize: 72, animation: 'bounceIn .5s ease' }}>{isOk ? '✅' : '❌'}</div>
        <div style={{ fontFamily: "'Fredoka One',cursive", fontSize: 28, color: '#fff' }}>
          {isOk ? 'Correto! 🎉' : 'Errou! 😅'}
        </div>
        {isOk ? (
          <>
            <div style={{ fontSize: 13, fontWeight: 800, textTransform: 'uppercase', letterSpacing: 1, color: 'rgba(255,255,255,.7)' }}>Pontos ganhos</div>
            <div style={{ fontFamily: "'Fredoka One',cursive", fontSize: 56, color: '#fbbf24', lineHeight: 1 }}>+{lastPts.toLocaleString('pt-BR')}</div>
            {streak >= 2 && <div style={{ fontSize: 16, fontWeight: 800, color: '#fbbf24' }}>🔥 {streak} acertos seguidos!</div>}
          </>
        ) : (
          <div style={{ fontSize: 15, color: 'rgba(255,255,255,.85)', fontWeight: 700 }}>
        ...
        <div className="score-box" style={{ marginTop: 8, width: 220 }}>
          <div style={{ fontSize: 12, fontWeight: 800, textTransform: 'uppercase', letterSpacing: 1, color: 'rgba(255,255,255,.6)', marginBottom: 2 }}>Total</div>
          <div className="score-num">{myScore.toLocaleString('pt-BR')}</div>
        </div>
        <p style={{ color: 'rgba(255,255,255,.5)', fontSize: 13, fontWeight: 700, marginTop: 8 }}>Aguardando próxima pergunta…</p>
      </div>
    )
  }
  ...
  return (
    <div className="game-bg" style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh' }}>
      ...
      {/* Top bar */}
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 14px', background: 'rgba(0,0,0,.15)' }}>
        <span style={{ fontWeight: 800, fontSize: 13, color: 'rgba(255,255,255,.7)' }}>{cqIdx + 1}/{questions.length}</span>
      ...
      {/* Progress */}
      <div className="progress-wrap" style={{ margin: '0 0 2px' }}>
      ...
      <div style={{ padding: '8px 14px 4px', display: 'flex', alignItems: 'center', gap: 10 }}>
        ...
        <span style={{ fontSize: 11, color: 'rgba(255,255,255,.6)', fontWeight: 700, flexShrink: 0 }}>{q.pontuacao}pts</span>
      </div>
      ...
      {/* Score footer */}
      <div style={{ padding: '10px 14px 20px', textAlign: 'center' }}>
        <div style={{ fontSize: 11, fontWeight: 800, textTransform: 'uppercase', letterSpacing: .5, color: 'rgba(255,255,255,.5)' }}>Pontuação</div>
        <div style={{ fontFamily: "'Fredoka One',cursive", fontSize: 26, color: '#fbbf24' }}>{myScore.toLocaleString('pt-BR')}</div>
      </div>
    </div>
  )
}
